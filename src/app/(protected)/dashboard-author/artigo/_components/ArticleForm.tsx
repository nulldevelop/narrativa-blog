...
import { X, Plus, Save, Send, Image as ImageIcon, Tag, Upload, Eye } from "lucide-react";
...
const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleSubmit = async (status: "draft" | "published", saveFirst = true) => {
    if (!formData.title) return toast.error("O título é obrigatório.");
    if (!content || content.length < 50) return toast.error("O conteúdo deve ter pelomeno 50 caracteres.");
    ...
    setLoading(true);
    try {
      const payload = { ...formData, content: content || "", tags, status };
      let result;
      if (initialData?.id) {
      ...
      if (result.success) {
        toast.success(status === "published" ? "Matéria publicada!" : "Rascunho salvo!");
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
      <div className="lg:col-span-2 space-y-6">
        <div className="space-y-2">
          <Label className="text-[0.65rem] font-black uppercase tracking-widest text-black/50">Título</Label>
          <Input
        ...
        <div className="space-y-2">
          <Label className="text-[0.65rem] font-black uppercase tracking-widest text-black/50">Subtítulo</Label>
          <Input
        ...
        <div className="space-y-2">
          <Label className="text-[0.65rem] font-black uppercase tracking-widest text-black/50">Conteúdo</Label>
      ...
      <aside className="space-y-6">
        <div className="bg-white border border-black/5 p-6 shadow-sm space-y-6">
          ...
          <div className="space-y-3">
            ...
            >
              <Send className="w-4 h-4 mr-2" /> {initialData?.id ? "Atualizar" : "Publicar Agora"}
            </Button>
        ...
        <div className="bg-white border border-black/5 p-6 shadow-sm space-y-6">
          <div className="space-y-2">
            <Label className="text-[0.65rem] font-black uppercase tracking-widest text-black/50">Categoria</Label>
            <Select 
          ...
          <div className="space-y-2">
            <Label className="text-[0.65rem] font-black uppercase tracking-widest text-black/50">Imagem de Capa</Label>
            <div className="space-y-2">
              <input
                type="file"
            ...
            {formData.coverImage && (
              <div className="aspect-video w-full mt-3 border border-narrativa-cinza-linha overflow-hidden">
            ...
            <div className="space-y-2 border-t border-black/5 pt-4">
              ...
              <div className="grid grid-cols-3 gap-2">
                {uploadedImages.map((url, i) => (
                  <div key={i} className="relative aspect-video border border-narrativa-cinza-linha overflow-hidden group cursor-pointer" onClick={() => { console.log("clicou na imagem", url); copyToClipboard(url); }}>
                    <img src={url} alt={`Imagem ${i + 1}`} className="w-full h-full object-cover" />
                    <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                      <span className="text-white text-[0.5rem] font-bold uppercase">Copiar</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="space-y-3 border-t border-black/5 pt-6">
            ...
            <div className="flex flex-wrap gap-1.5 pt-2">
              {tags.map((t) => (
                <Badge key={t} className="rounded-none text-[0.6rem] font-bold uppercase bg-black/5 hover:bg-narrativa-vermelho hover:text-white transition-colors py-1 px-2 flex items-center gap-1">
                    {t} 
                    <button type="button" onClick={() => setTags((prev) => prev.filter((tag) => tag !== t))}>
                      <X className="w-3 h-3" />
                    </button>
                </Badge>
              ))}
