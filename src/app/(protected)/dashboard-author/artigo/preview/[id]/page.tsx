      ...
      <div className="sticky top-0 z-50 bg-narrativa-preto px-8 py-3 flex items-center justify-between border-b border-white/10">
        <div className="flex items-center gap-4">
          <div className="flex flex-col">
            <span className="text-white/40 text-[0.55rem] font-black uppercase tracking-[0.2em]">
              Modo de Visualização
            </span>
            <span className="text-white text-[0.7rem] font-bold uppercase tracking-wider">
              Preview Editorial
            </span>
          </div>
        ...
        <div className="flex items-center gap-3">
          <Button
            asChild
            variant="ghost"
            className="text-white hover:bg-white/10 rounded-none h-10 px-6 text-[0.65rem] font-bold uppercase tracking-widest"
          >
          ...
          <div className="flex flex-col">
            <div className="flex items-center gap-4 mb-8">
              <span className="text-[0.65rem] tracking-[0.25em] uppercase text-narrativa-vermelho font-black">
                Política ·{' '}
                {article.categoryId ? 'Categoria Selecionada' : 'Geral'}
              </span>
            </div>
          ...
          <div className="relative w-full aspect-[4/3] lg:aspect-[16/11] overflow-hidden rounded-[4px] bg-narrativa-cinza-claro shadow-2xl">
            {article.coverImage ? (
              ...
              <div className="w-full h-full flex items-center justify-center text-slate-400 italic font-serif">
                Nenhuma imagem de capa definida
              </div>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-[1fr_300px] gap-24 items-start max-md:grid-cols-1">
          ...
          <aside className="flex flex-col gap-10">
            <div className="pb-8 border-b border-narrativa-cinza-linha">
              <p className="text-[0.65rem] tracking-[0.2em] uppercase text-narrativa-vermelho font-black mb-6">
                Informações do Post
              </p>
              <div className="space-y-4">
                <div>
                  <span className="block text-[0.55rem] uppercase text-slate-400 font-bold mb-1">
                    Status Editorial
                  </span>
                  <span className="text-[0.75rem] font-bold text-slate-700 uppercase">
                    {article.status}
                  </span>
                </div>
                <div>
            ...
            <div className="p-6 bg-slate-50 border border-slate-100">
              <p className="text-[0.7rem] leading-relaxed text-slate-500 italic font-serif">
                &quot;Este é um modo de visualização segura. Nenhuma alteração
                feita aqui será salva.&quot;
              </p>
            </div>
          </aside>
        </div>
