      ...
      <div className="flex-1 flex flex-col lg:flex-row max-w-[1600px] mx-auto w-full p-8 gap-8">
        ...
        <aside className="w-full lg:w-64 flex flex-col gap-2 shrink-0">
          ...
          <nav className="space-y-1">
            ...
            <Link
              href="/dashboard-editor/materias"
            ...
            <Link
              href="/dashboard-editor/categorias"
              className="flex items-center gap-3 px-3 py-2.5 text-black/60 hover:bg-black/5 font-bold text-[0.8rem] uppercase tracking-wider transition-all"
            >
          ...
          <div className="flex items-end justify-between border-b border-black/10 pb-6 flex-wrap gap-4">
            <div>
              ...
              <p className="text-[0.9rem] text-black/40 font-light">
                Organização e supervisão da produção política.
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <Card className="rounded-none border-none shadow-sm">
              <CardContent className="p-5">
                <p className="text-[0.6rem] font-black tracking-widest uppercase text-black/30 mb-1">
                  Total de Peças
                </p>
            ...
            <Card className="rounded-none border-none shadow-sm">
              <CardContent className="p-5">
                <p className="text-[0.6rem] font-black tracking-widest uppercase text-green-600/60 mb-1">
                  Publicadas
                </p>
                <div className="flex items-baseline justify-between">
            ...
            <Card className="rounded-none border-none shadow-sm bg-narrativa-vermelho/5">
              <CardContent className="p-5">
                <p className="text-[0.6rem] font-black tracking-widest uppercase text-narrativa-vermelho/60 mb-1">
                  Total de Acessos
                </p>
                <div className="flex items-baseline justify-between">
                  <span className="text-2xl font-black text-narrativa-vermelho">
                    {totalViews._sum.views?.toLocaleString('pt-BR') || 0}
                  </span>
            ...
            <Card className="rounded-none border-none shadow-sm">
              <CardContent className="p-5">
                <p className="text-[0.6rem] font-black tracking-widest uppercase text-narrativa-dourado/60 mb-1">
                  Em Rascunho
                </p>
                <div className="flex items-baseline justify-between">
                  <span className="text-2xl font-black text-narrativa-dourado">
                    {draftArticles}
                  </span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-1 space-y-4">
              <h4 className="text-[0.75rem] font-black tracking-[0.2em] uppercase text-black/40 px-1 flex items-center gap-2">
                <TrendingUp className="w-3.5 h-3.5 text-narrativa-vermelho" />{' '}
                Top Performance
              </h4>
              <Card className="rounded-none border-none shadow-sm">
                <CardContent className="p-5 space-y-4">
                  {topArticles.map((article, i) => (
                    <div
                      key={article.id}
                      className="flex flex-col gap-1 border-b border-black/5 last:border-0 pb-3 last:pb-0"
                    >
                      <h5 className="text-[0.8rem] font-bold text-narrativa-preto line-clamp-1">
                        {article.title}
                      </h5>
                      <div className="flex items-center justify-between">
                        <span className="text-[0.6rem] text-black/30 uppercase font-black">
                          {article.author.name}
                        </span>
              ...
              <div className="flex items-center justify-between px-1">
                ...
                <div className="relative w-64">
                  ...
                  <Input
                    ...
                    className="pl-9 h-9 rounded-none border-black/5 text-[0.75rem]"
                  />
                </div>
              </div>
                    ...
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="bg-black/[0.02] border-b border-black/5">
                          <th className="px-6 py-4 text-[0.65rem] font-black tracking-widest uppercase text-black/40">
                            Título da Matéria
                          </th>
                          <th className="px-6 py-4 text-[0.65rem] font-black tracking-widest uppercase text-black/40">
                            Autor
                        ...
                        {recentArticles.length > 0 ? (
                          recentArticles.map((article) => (
                            ...
                            >
                              <td className="px-6 py-4">
                                <p className="text-[0.85rem] font-bold text-narrativa-preto line-clamp-1">
                                  {article.title}
                                </p>
                              ...
                              <td className="px-6 py-4 text-right">
                                <Button
                                  variant="ghost"
                                  size="icon"
                                  className="group-hover:text-narrativa-vermelho transition-colors"
                                >
                          ...
                          <tr>
                            <td colSpan={5} className="px-6 py-12 text-center">
                              ...
                              <p className="text-[0.85rem] text-black/40 italic font-serif">
                                Nenhuma matéria produzida ainda.
                              </p>
                              <Button
                                variant="link"
                                className="text-narrativa-vermelho text-[0.7rem] font-black uppercase tracking-widest mt-2"
                              >
