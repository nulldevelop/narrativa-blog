...
export async function SidebarHome({ tags }: SidebarHomeProps) {
  ...
  return (
    <aside className="flex flex-col gap-4 lg:gap-5 xl:gap-7 min-w-0" aria-label="Coluna lateral">
      {/* Curtas & Diretas */}
      <div className="bg-[#0b0b0b] p-4 xl:p-5 shadow-xl w-full overflow-hidden">
        <h3 className="text-[0.78rem] font-bold tracking-[0.2em] uppercase text-white mb-2">
          CURTAS & DIRETAS
        </h3>
        <div className="w-10 h-[2px] bg-[#e63030] mb-3 xl:mb-5" />

        <div className="overflow-y-auto pr-2 flex flex-col gap-3 lg:gap-4 max-h-[300px] lg:max-h-[360px] xl:max-h-[480px] custom-scrollbar">
      ...
      {/* Newsletter */}
      <div className="border border-narrativa-cinza-linha p-4 xl:p-6">
        <h3 className="text-[0.92rem] xl:text-[1.05rem] font-bold mb-2 pb-2 border-b border-narrativa-cinza-linha">
          Receba avisos de novidades!
        </h3>
        <p className="text-[0.8rem] text-narrativa-cinza-texto leading-[1.6] font-light mb-3">
      ...
      {/* Quote */}
      <div className="border-l-[3px] border-narrativa-dourado pl-4 py-1">
        <p className="italic text-[0.82rem] xl:text-[0.92rem] text-narrativa-cinza-texto leading-[1.6]">
      ...
      {cotidianoArticles.length > 0 && (
        <div className="flex flex-col gap-4">
          <p className="text-[0.65rem] font-bold tracking-[0.15em] uppercase text-narrativa-vermelho">
            Cotidiano
          </p>

          {cotidianoArticles.slice(0, 3).map((article) => (
            <Link
              key={article.id}
              href={`/artigo/${article.slug}`}
              className="group block border-b border-narrativa-cinza-linha pb-4 last:border-0"
            >
              <div className="relative w-full mb-2 overflow-hidden transition-all duration-500" style={{ aspectRatio: '16/7', minHeight: '130px' }}>
                <Image
              ...
              <h4 className="text-[clamp(1rem,2.5vw,1.1rem)] font-bold text-narrativa-preto leading-tight group-hover:text-narrativa-vermelho transition-colors uppercase tracking-tight">
                {article.title}
              </h4>
            </Link>
          ))}
